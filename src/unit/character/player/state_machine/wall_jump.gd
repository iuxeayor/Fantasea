...
func enter() -> void:
	...
	character.velocity.y = Constant.WALL_JUMP_VERTICAL_VELOCITY
	# 影响跳跃的计时器
	...
	# 墙跳横向速度基于方向
	character.velocity.x = (Constant.WALL_JUMP_HORIZONTAL_SPEED
		if character.face_direction == Constant.Direction.RIGHT
		else -Constant.WALL_JUMP_HORIZONTAL_SPEED)
