...
var milk_unit_price: int = 10

@export var handle_scene: int = 0
@onready var shop_dialog: DialogContent = $Dialog/Shop

func _ready() -> void:
	super ()
	milk_unit_price = Util.get_max_health(
		Status.player_status.number_collection.get("health_chip", 0)) * 2
	_handle_text("language")
	Config.config_changed.connect(_handle_text)
	# 仅在Boss未击败时出现
	await owner.ready
	if Status.scene_status.scene_explore[handle_scene] == Constant.ExploreStatus.COMPLETE:
		queue_free()
